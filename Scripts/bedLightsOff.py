...
def setLevel(id, value):
    ...
    print (MESSAGE)
    directorConn.sendall((MESSAGE + '\0').encode())
    directorConn.close()
...
def getLevel(id):
    ...
    directorConn.connect((TCP_IP, TCP_PORT))
    MESSAGE = (
        '<c4soap name="GetVariable" async="False"><param name = "deviceid" type = "INT">%d</param><param name = "variableid" type = "INT">1001</param></c4soap>'
        % (id)
    )
    directorConn.sendall(MESSAGE.encode() + b"\0")
    data = directorConn.recv(BUFFER_SIZE)
    print(data)
    directorConn.close()
    data = BeautifulSoup(data[:-1], "lxml-xml")
